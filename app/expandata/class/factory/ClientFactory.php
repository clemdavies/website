...
Class ClientFactory extends ExpDatabase{
  ...
  public function update($clientObject){
    ...
    try{
      ...
      $nameRecord->id    = $clientObject->getName()->getId();
      ...
      $contactRecord->id     = $clientObject->getContact()->getId();
      ...
      $postcodeRecord->id = $clientObject->getPostalAddress()->getPostcode()->getId();

      if ( $postcodeRecord->number != $clientObject->getPostalAddress()->getPostcode()->getNumber() ){
        // this postcode has been updated. BUT don't delete the old postcode, as it may still be used on another object.
        // :. dehydrate mapper
        $postcodeRecord->reset();
        // create new record
        $postcodeRecord->number = $clientObject->getPostalAddress()->getPostcode()->getNumber();
        $postcodeRecord->save();
        // update postcode object with new id
        $clientObject->getPostalAddress()->getPostcode()->setId($postcodeRecord->id);
      }
      // ELSE postcode not updated :. do nothing

      // dehydrate mapper object
      $postcodeRecord->reset();
      $postcodeRecord->id = $clientObject->getStreetAddress()->getPostcode()->getId();
      if ( $postcodeRecord->number != $clientObject->getStreetAddress()->getPostcode()->getNumber() ){
        $postcodeRecord->reset();
      ...
      // postcode id is now updated, :. it should be okay if suburb name is redone.
      $suburbRecord = new DB\SQL\Mapper(self::$db,'suburb');
      $suburbRecord->id = $clientObject->getPostalAddress()->getSuburb()->getId();
      if ( $suburbRecord->name != $clientObject->getPostalAddress()->getSuburb()->getName() ){
        // suburb name updated

        // SHOULD query for existence of suburb id being used in address records
        // if it is not being used then it is okay to update it. otherwise create new record.
      ...
      if ($suburbRecord->postcode_id != $clientObject->getPostalAddress()->getPostcode()->getId()){
        // suburb postcode updated
        // :. update record
        $suburbRecord->postcode_id = $clientObject->getPostalAddress()->getPostcode()->getId();
        $suburbRecord->save();
      }
      ...
      // dehydrate mapper object
      $suburbRecord->reset();
      $suburbRecord->id = $clientObject->getStreetAddress()->getSuburb()->getId();
      if ( $suburbRecord->name != $clientObject->getStreetAddress()->getSuburb()->getName() ){
        // suburb name updated
      ...
      if ($suburbRecord->postcode_id != $clientObject->getStreetAddress()->getPostcode()->getId()){
        // suburb postcode updated
        // :. update record
        $suburbRecord->postcode_id = $clientObject->getStreetAddress()->getPostcode()->getId();
        $suburbRecord->save();
      ...
      $addressRecord->id = $clientObject->getPostalAddress()->getId();
      ...
      if (!( $clientObject->getPostalAddress() === $clientObject->getStreetAddress() )){

        // dehydrate mapper object
        $addressRecord->reset();
        $addressRecord->id = $clientObject->getStreetAddress()->getId();
      ...
      $clientRecord->id          = $clientObject->getId();
      ...
      // clean up any records based on ids that are not used
      // background process, or use same request to process this?
